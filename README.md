...
I will use the plain (P3) PPM raster image format to represent images and a tuple of three integers from 0 to 255 (Red, Green, Blue) to represent the colors/pixels of an image. The k-means algorithm begins with randomly assigned clusters of colors (completely random colors with no relation to the inputted image). Then it takes each pixel in the image and assigns it to the most similiar color cluster. After that's done, the algorithm will average out the pixels in each color cluster, and set that averaged color as the new color for the color cluster. The algorithm repeats itself until it finds a combination of colors that cannot be improved. Finally, after the algorithm's obtained the most accuarate clusters of colors, it takes each pixel in the original image, finds the closest color cluster, and sets that clolor cluster to the new pixel in a new image. This new image will be outputed with only the colors in the color clusters. To understand this, let's use an example. 
...
# What I Learned 
I learned the k-means machine learning algorithm and how to represent images in different formats. I also became very familiar with some of Python's data structures and useful functions. 
