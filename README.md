...
Imagine you inputted an image full of purple, blue, and red. You set the k-means clustering to three which means you want three clusters of colors. The algorithm will begin with completely random colors of color clusters: let's say, yellow, pink, and green. Then it takes all of the pixels in the image and matches it with the closest color cluster (in our case, yellow, pink, or green). After averaging the pixels in the clusters and repeating the process over and over again until the clusters can't get any more accurate, the algorithm finally gets something along the lines of purple, blue, and red as the final cluster of colors. Then the algorithm takes each of the pixels in the original image and assigns the pixel to the closest cluster of purple, blue, or red, and then outputs the final image. So essentially, if you want k-means of three, then your final image will only consist of the three most core colors of your image.
