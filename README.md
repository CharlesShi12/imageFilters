...
The k-means algorithm begins with randomly generated colors that represent the centers of clusters of colors (completely random colors with no relation to the inputted image). Then it takes each pixel in the image and assigns it to the most similar color cluster. After that's done, the algorithm will average out the pixels in each color cluster and set that averaged color as the color to represent the center of the new color cluster. The algorithm repeats itself until it finds a combination of color clusters that cannot be improved. Finally, after the algorithm has obtained the most accurate clusters of colors, it will take each pixel in the original image, find the closest color cluster, and set the center color of that color cluster to the pixel in a new image. This new image will be outputted with only the center colors of the final color clusters.
